...
When in org mode, we save the modified headings in a +temporary buffer+ variable, then before saving the buffer, we use this information to let the ~format-code-before-save-buffer~ function now where to format.
*** Code
:LOGBOOK:
...
(defun open-org-file-from-project ()
  ...
  (let ((file-location (concat org-directory "/" 
                               (with-temp-buffer
                                 ;; We may want to refactor `projectile-project-root' to a custom function
                                 ;; so that this function can live without installing projectile
                                 (insert-file-contents (concat (projectile-project-root) "/org-entry"))
                                 (buffer-string))
                               )))
